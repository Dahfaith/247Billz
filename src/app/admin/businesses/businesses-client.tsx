...
import { toggleBusinessSuspension, adminUpgradeBusinessPlan } from '@/app/actions/admin'
...
export function AdminBusinessesClient({ initialBusinesses }: { initialBusinesses: any[] }) {
  const [searchQuery, setSearchQuery] = useState('')
  const [isLoading, setIsLoading] = useState<string | null>(null)
  const router = useRouter()

  const filteredBusinesses = (initialBusinesses || []).filter((b: any) => {
    const query = searchQuery.toLowerCase()
    return (
      b.name?.toLowerCase().includes(query) ||
      b.email?.toLowerCase().includes(query) ||
      b.subscription_tier?.toLowerCase().includes(query) ||
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["ID", "Company Name", "Email", "Phone", "Plan", "Status", "Joined"]
  ...
  const handleUpgrade = async (id: string, tier: 'starter' | 'pro' | 'business') => {
    setIsLoading(`upgrade-${id}`)
    try {
      await adminUpgradeBusinessPlan(id, tier)
      toast.success(`Business upgraded to ${tier} successfully!`)
      router.refresh()
    } catch (e: any) {
      toast.error(e.message || "Failed to upgrade business")
    } finally {
      setIsLoading(null)
    }
  }

  const handleToggleSuspension = async (id: string, status: string) => {
    setIsLoading(`suspend-${id}`)
    try {
      await toggleBusinessSuspension(id, status || 'active')
      const newStatus = status === 'suspended' ? 'activated' : 'suspended'
      toast.success(`Business has been ${newStatus}.`)
      router.refresh()
    } catch (e: any) {
      toast.error(e.message || "Failed to toggle suspension")
    } finally {
      setIsLoading(null)
    }
  }
                  ...
                  <TableRow key={business.id} className="border-[#E2E8F0] dark:border-slate-800">
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-[#F97316] ${business.status === 'suspended' ? 'bg-red-100 dark:bg-red-900/30 text-red-500 border border-red-200 dark:border-red-800' : 'bg-gradient-to-br from-orange-100 to-orange-50 dark:from-slate-800 dark:to-slate-900 border border-orange-200 dark:border-slate-700'}`}>
                          {business.name.substring(0, 1).toUpperCase()}
                        </div>
                        <div>
                          <div className={`font-medium ${business.status === 'suspended' ? 'text-slate-400 line-through' : 'text-slate-900 dark:text-slate-100'}`}>{business.name}</div>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" className="h-8 w-8 p-0" disabled={isLoading?.includes(business.id)}>
                        ...
                        <DropdownMenuContent align="end" className="w-48 bg-white dark:bg-[#0F172A] border-[#E2E8F0] dark:border-slate-800">
                          ...
                          <DropdownMenuItem className="cursor-pointer" onClick={() => toast("Impersonation coming soon!", { icon: '🚧' })}>
                            <UserCog className="mr-2 h-4 w-4" />
                            Impersonate Login
                          </DropdownMenuItem>
                          <DropdownMenuSeparator className="bg-[#E2E8F0] dark:bg-slate-800" />
                          <DropdownMenuItem className="cursor-pointer text-purple-600 focus:text-purple-600" onClick={() => handleUpgrade(business.id, 'pro')}>
                            <ArrowUpCircle className="mr-2 h-4 w-4" />
                            Upgrade to Pro
                          </DropdownMenuItem>
                          <DropdownMenuItem className="cursor-pointer text-blue-600 focus:text-blue-600" onClick={() => handleUpgrade(business.id, 'business')}>
                            <ArrowUpCircle className="mr-2 h-4 w-4" />
                            Upgrade to Business
                          </DropdownMenuItem>
                          <DropdownMenuItem 
                            className={`cursor-pointer ${business.status === 'suspended' ? 'text-green-600 focus:text-green-600' : 'text-red-600 focus:text-red-600'}`}
                            onClick={() => handleToggleSuspension(business.id, business.status || 'active')}
                          >
